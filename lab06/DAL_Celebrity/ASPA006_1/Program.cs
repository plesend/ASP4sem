    ...
    public class Program 
    {
        public static void Main(string[] args)
        {
            ...
            celebrities.MapDelete("/{id:int:min(1)}", (IRepository repo, int id) =>
            {
                try
                {
                    if (!repo.DelCelebrity(id)) throw new DeleteCelebrityException($"Delete error for id: {id}");
                    return Results.Ok();
                }
                catch (Exception ex)
                {
            ...
            lifeevents.MapDelete("/{id:int:min(1)}", (IRepository repo, int id) =>
            {
                try
                {
                    if (!repo.DelLifeevent(id)) throw new DeleteCelebrityException($"Delete error for Lifeevent id: {id}");
                    return Results.Ok();
                }
                catch (Exception ex)
                {
